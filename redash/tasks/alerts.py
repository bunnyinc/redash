...
@celery.task(name="redash.tasks.check_alert", bind=True, base=BaseTask)
def check_alert(self, alert_id):
    from redash.wsgi import app
    logger.debug("Checking alert %d", alert_id)
    ...
    if new_state != alert.state or (alert.state == models.Alert.TRIGGERED_STATE and passed_rearm_threshold ):
        ...
        """.format(host=base_url(alert.query.org), alert_id=alert.id, query_id=alert.query.id)
        ...
        if settings.WEBHOOK_ENDPOINT:
            notify_webhook(alert, alert.query, html, new_state)
